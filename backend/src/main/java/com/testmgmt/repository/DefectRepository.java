...
public interface DefectRepository extends JpaRepository<Defect, UUID> {
    ...
    // JIRA integration
    java.util.Optional<Defect> findByJiraIssueKey(String jiraIssueKey);
    java.util.Optional<Defect> findByJiraIssueId(String jiraIssueId);

    @Query(value = "SELECT project_id, status, COUNT(*) FROM defects " +
                   "WHERE project_id IN :projectIds GROUP BY project_id, status",
           nativeQuery = true)
    List<Object[]> countStatusesForProjects(@Param("projectIds") List<UUID> projectIds);
}
