...
public class TestCaseWorkflowService {
    ...
    // ── Forward DRAFT / PASSED → PENDING_SME_REVIEW ───────────────────
    @Transactional
    @Caching(evict = {
    ...
    public TestCaseResponse forwardToSME(UUID testCaseId, UUID smeId, String managerEmail) {
        ...
        if (List.of(TestStatus.SIGNED_OFF, TestStatus.UAT_PASSED,
                    TestStatus.DEPRECATED, TestStatus.PENDING_SME_REVIEW,
                    TestStatus.RELEASE_REQUESTED, TestStatus.RELEASED).contains(tc.getStatus()))
