...
export const callApi = {
  list:       (projectId, params = {}) =>
    api.get('/api/v1/calls', { params: { projectId, ...params } }),
  get:        (id)         => api.get(`/api/v1/calls/${id}`),
  upcoming:   (projectId)  => api.get('/api/v1/calls/upcoming', { params: { projectId } }),
  myCalls:    (projectId)  => api.get('/api/v1/calls/my-calls',  { params: { projectId } }),
  summary:    (projectId)  => api.get('/api/v1/calls/summary',   { params: { projectId } }),
  create:     (data)       => api.post('/api/v1/calls', data),
  update:     (id, data)   => api.put(`/api/v1/calls/${id}`, data),
  updateStatus:(id, data)  => api.patch(`/api/v1/calls/${id}/status`, data),
  complete:   (id, data)   => api.post(`/api/v1/calls/${id}/complete`, data),
  delete:     (id)         => api.delete(`/api/v1/calls/${id}`),
};

// ── Central Repository Module ───────────────────────────────────────────────
export const repositoryApi = {
  list:       (projectId, category) =>
    api.get(`/api/v1/repository/projects/${projectId}/documents`,
            { params: category ? { category } : {} }),
  upload:     (projectId, formData) =>
    api.post(`/api/v1/repository/projects/${projectId}/documents`, formData,
             { headers: { 'Content-Type': 'multipart/form-data' } }),
  download:   (docId)    => api.get(`/api/v1/repository/documents/${docId}/download`,
                                    { responseType: 'blob' }),
  archive:    (docId)    => api.patch(`/api/v1/repository/documents/${docId}/archive`),
  delete:     (docId)    => api.delete(`/api/v1/repository/documents/${docId}`),
  categories: ()         => api.get('/api/v1/repository/categories'),
};

// ── Release Management ──────────────────────────────────────────────────────
export const releaseApi = {
  request:    (testCaseId, data) => api.post(`/api/v1/releases/testcases/${testCaseId}/request`, data),
  action:     (releaseId, data)  => api.patch(`/api/v1/releases/${releaseId}/action`, data),
  pending:    ()                 => api.get('/api/v1/releases/pending'),
  history:    (testCaseId)       => api.get(`/api/v1/releases/testcases/${testCaseId}/history`),
  mine:       ()                 => api.get('/api/v1/releases/mine'),
};
